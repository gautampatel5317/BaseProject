<div class="form-group {{ $errors->has('name') ? 'has-error' : '' }}">
    <label for="name" class = "required">{{ trans('global.category_name') }}</label>
...
<div class="form-group">
    ...
    @php
        $selected_status = ( old('status') != "" ? old('status') : (isset($category) ? $category->status : '1' )  );
    @endphp
...
<div class="card-footer text-center">
    <a href="{{ route('admin.category.index') }}" class="btn btn-danger ml-2">Cancel</a>
    <input class="btn btn-primary" type="submit" value="{{ isset($category)  ?  trans('global.update') :trans('global.save') }}">
</div>
